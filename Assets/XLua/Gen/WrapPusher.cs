...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<MyType.PlayerType>(translator.PushMyTypePlayerType, translator.Get, translator.UpdateMyTypePlayerType);
        ...
        int MyTypePlayerType_TypeID = -1;
		int MyTypePlayerType_EnumRef = -1;
        
        public void PushMyTypePlayerType(RealStatePtr L, MyType.PlayerType val)
        {
            if (MyTypePlayerType_TypeID == -1)
            {
			    bool is_first;
                MyTypePlayerType_TypeID = getTypeId(L, typeof(MyType.PlayerType), out is_first);
				
				if (MyTypePlayerType_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(MyType.PlayerType));
				    MyTypePlayerType_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, MyTypePlayerType_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, MyTypePlayerType_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for MyType.PlayerType ,value="+val);
            }
			
			LuaAPI.lua_getref(L, MyTypePlayerType_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out MyType.PlayerType val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != MyTypePlayerType_TypeID)
				{
				    throw new Exception("invalid userdata for MyType.PlayerType");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for MyType.PlayerType");
                }
				val = (MyType.PlayerType)e;
                
            }
            else
            {
                val = (MyType.PlayerType)objectCasters.GetCaster(typeof(MyType.PlayerType))(L, index, null);
            }
        }
		
        public void UpdateMyTypePlayerType(RealStatePtr L, int index, MyType.PlayerType val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for MyType.PlayerType ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(MyType.PlayerType[]))
			{
			    MyType.PlayerType[] array = obj as MyType.PlayerType[];
				translator.PushMyTypePlayerType(L, array[index]);
				return true;
			}
