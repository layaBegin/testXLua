...
namespace XLua.CSObjectWrap
{
    ...
    public class MyTypePlayerTypeWrap
    {
		public static void __Register(RealStatePtr L)
        {
		    ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
		    Utils.BeginObjectRegister(typeof(MyType.PlayerType), L, translator, 0, 0, 0, 0);
			Utils.EndObjectRegister(typeof(MyType.PlayerType), L, translator, null, null, null, null, null);
			
			Utils.BeginClassRegister(typeof(MyType.PlayerType), L, null, 5, 0, 0);

            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "ZhangShan", MyType.PlayerType.ZhangShan);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "LiShi", MyType.PlayerType.LiShi);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "WangWu", MyType.PlayerType.WangWu);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "ZhaoLiu", MyType.PlayerType.ZhaoLiu);
            

			Utils.RegisterFunc(L, Utils.CLS_IDX, "__CastFrom", __CastFrom);
            
            Utils.EndClassRegister(typeof(MyType.PlayerType), L, translator);
        }
        ...
        static int __CastFrom(RealStatePtr L)
		{
            ...
            if (lua_type == LuaTypes.LUA_TNUMBER)
            {
                translator.PushMyTypePlayerType(L, (MyType.PlayerType)LuaAPI.xlua_tointeger(L, 1));
            }
			
            else if(lua_type == LuaTypes.LUA_TSTRING)
            {

			    if (LuaAPI.xlua_is_eq_str(L, 1, "ZhangShan"))
                {
                    translator.PushMyTypePlayerType(L, MyType.PlayerType.ZhangShan);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "LiShi"))
                {
                    translator.PushMyTypePlayerType(L, MyType.PlayerType.LiShi);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "WangWu"))
                {
                    translator.PushMyTypePlayerType(L, MyType.PlayerType.WangWu);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "ZhaoLiu"))
                {
                    translator.PushMyTypePlayerType(L, MyType.PlayerType.ZhaoLiu);
                }
				else
                {
                    return LuaAPI.luaL_error(L, "invalid string for MyType.PlayerType!");
                }
            ...
            {
                return LuaAPI.luaL_error(L, "invalid lua type for MyType.PlayerType! Expect number or string, got + " + lua_type);
            }
